...
'users.html':{
  ...
  rows:[
    ...
    ['UserProviderInterface','find(), findByEmail(), all()','Read operations a custom provider must implement.'],['UserProviderInterface','create(), update(), delete()','Write operations a custom provider must implement.'],['UserFactoryInterface','make(row, map)','Extension point for converting a provider row into a user object.']
  ]},
'input.html':{
...
'messaging.html':{
  ...
  rows:[
    ...
    ['Attachment','fromPath(path, name?, mime?)','Create an attachment from a file path.'],['Attachment','fromData(data, name, mime?)','Create an in-memory attachment.'],
    ['Template','render(data)','Replace template placeholders and return a Message.'],['DeliveryResult','successful, channel, messageId, error, meta','Read delivery success/failure and provider result information.'],
