...
/*
 |--------------------------------------------------------------------------
 | Shared Prefab interoperability bootstrap
 |--------------------------------------------------------------------------
 |
 | Every standalone Prefab package ships this guarded bootstrap so no separate
 | Core package is required. The first loaded Prefab package defines these
 | classes; later packages detect that they already exist and reuse them.
 |
 */
...
if (!class_exists(PrefabRuntime::class, false)) {
    final class PrefabRuntime
    {
        ...
        /**
         * Register a module instance and immediately re-run configuration for
         * all declared modules so declaration order remains flexible.
         */
        ...
        public static function emitLog(array $entry): void
        {
            ...
            if ($logs && method_exists($logs, 'record')) {
                $logs->record($entry);
            }
        }
        ...
        public static function actorId(): int|string|null
        {
            ...
            return ($auth && method_exists($auth, 'id'))
                ? $auth->id()
                : null;
        }
