...
/*
 |--------------------------------------------------------------------------
 | PROJECT-OWNED DATABASE TABLE
 |--------------------------------------------------------------------------
 | The application owns this users table. Prefab Users DOES NOT create it.
 | Prefab only maps to and uses the existing table/columns.
 */
...
/*
 |--------------------------------------------------------------------------
 | COMMON PREFAB CONFIGURATION
 |--------------------------------------------------------------------------
 | This is declared BEFORE the modules.
 |
 | - Main database is the shared/default database.
 | - Users maps to the PROJECT'S app_users table.
 | - Permissions uses the same main database and creates its own table.
 | - Logs is NOT overridden here; it will be overridden locally below.
 |
 | Module-local configuration never changes this shared configuration.
 */
PrefabConfig::set([
    'database' => $mainDb,
    'modules' => [
...
/*
 ...
 |--------------------------------------------------------------------------
 | No central Core object is needed.
 |
 | Users uses the common $mainDb + project-owned app_users mapping.
 | Auth finds and integrates with Prefab Users automatically.
 | Permissions uses the common $mainDb and creates prefab_subject_permissions.
 | Logs explicitly uses $logDb, so only Logs uses the separate database.
 |
 | When Logs is declared last, the runtime performs its final integration pass.
 | Normal feature calls afterwards use the already-resolved references.
 */
...
/*
 | Optional variations:
 |
 | Use a different Permissions database only:
 | $permissions = new PermissionManager(['database' => $securityDb]);
 |
 | Use a custom Users provider instead of the project table mapping:
 | $users = new UserManager($customUserProvider);
 |
 | Those local overrides affect only that module. Automatic integration with
 | the other Prefab modules remains active.
 */
...
<head>
    ...
    <title>Prefab Real Database Integration Test</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.8/dist/css/bootstrap.min.css" rel="stylesheet">
</head>
<body class="bg-body-tertiary">
<nav class="navbar navbar-dark bg-dark"><div class="container"><span class="navbar-brand">Tihloh Prefab — Real Database Integration</span></div></nav>
    ...
    <div class="row g-4">
        <div class="col-lg-4">
            <div class="card mb-4">
                ...
                <div class="card-body">
                    <?php if ($auth->check()): ?>
                        <p>Signed in as <strong><?= e($current?->name ?? $auth->id()) ?></strong></p>
                        <form method="post"><button name="action" value="logout" class="btn btn-outline-danger w-100">Sign out</button></form>
                    <?php else: ?>
            ...
            <div class="card">
                <div class="card-header fw-semibold">Database ownership</div>
                <div class="card-body small">
                    <p><strong>Project:</strong> <code>app_users</code></p>
                    <p><strong>Prefab Permissions:</strong> <code>prefab_subject_permissions</code></p>
                    <p class="mb-0"><strong>Prefab Logs:</strong> <code>prefab_logs</code> in separate DB</p>
                </div>
            </div>
        </div>
        ...
        <div class="col-lg-8">
            <div class="card mb-4">
                <div class="card-header fw-semibold">Project Users</div>
                    ...
                    <table class="table align-middle mb-0">
                        ...
                        <?php foreach ($users->all() as $user): ?>
                            <tr>
                                ...
                                <td><?= e($user->name) ?></td>
                                <td><?= e($user->email) ?></td>
                                <td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="?user=<?= e($user->id) ?>">Manage</a></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
                    </table>
            ...
            <?php if ($selected): ?>
            <div class="card mb-4">
                <div class="card-header fw-semibold">Manage <?= e($selected->name) ?></div>
                <div class="card-body">
                    <form method="post" class="row g-2 mb-3">
                        <input type="hidden" name="user_id" value="<?= e($selected->id) ?>">
                        <div class="col"><input class="form-control" name="name" value="<?= e($selected->name) ?>"></div>
                        <div class="col-auto"><button name="action" value="rename" class="btn btn-outline-primary">Rename</button></div>
                    </form>

                    <table class="table table-sm align-middle">
                        <thead><tr><th>Permission</th><th>Effective</th><th>Source</th><th>Override</th></tr></thead>
                        <tbody>
                        <?php foreach ($permissions->definitions() as $id => $definition): $result = $resolved[$id]; ?>
                            <tr>
                                <td><?= e($definition['name'] ?? $id) ?><br><code class="small"><?= e($id) ?></code></td>
                                <td><span class="badge text-bg-<?= $result->allowed ? 'success' : 'danger' ?>"><?= $result->allowed ? 'ALLOW' : 'DENY' ?></span></td>
                                <td><?= e($result->source) ?></td>
                                <td>
                                    <form method="post" class="btn-group btn-group-sm">
                                        <input type="hidden" name="user_id" value="<?= e($selected->id) ?>">
                                        <input type="hidden" name="permission" value="<?= e($id) ?>">
                                        <button name="action" value="allow" class="btn btn-outline-success">Allow</button>
                                        <button name="action" value="deny" class="btn btn-outline-danger">Deny</button>
                                        <button name="action" value="clear" class="btn btn-outline-secondary">Clear</button>
                                    </form>
                                </td>
                            </tr>
                        <?php endforeach; ?>
            ...
            <div class="card">
                <div class="card-header fw-semibold">Activity stored in logs.sqlite</div>
                <div class="table-responsive">
                    <table class="table table-hover align-middle mb-0">
                        <thead><tr><th>#</th><th>Action</th><th>Actor</th><th>Subject</th><th>Message</th></tr></thead>
                        <tbody>
                        <?php foreach ($recentLogs as $log): ?>
                            <tr>
                                ...
                                <td><?= e($log['subject_type']) ?> #<?= e($log['subject_id'] ?? '—') ?></td>
                                <td><?= e($log['message'] ?? '') ?></td>
                            </tr>
                        <?php endforeach; ?>
                        </tbody>
