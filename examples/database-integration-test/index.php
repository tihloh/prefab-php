...
/* Project-owned tables. Prefab Users maps to them but does not own them. */
$mainDb->exec(<<<SQL
...
if ((int) $mainDb->query('SELECT COUNT(*) FROM app_users')->fetchColumn() === 0) {
    ...
    $statement->execute(['Demo Admin', 'demo@example.com', password_hash('password123', PASSWORD_DEFAULT)]);
    $statement->execute(['Test User', 'test@example.com', password_hash('password123', PASSWORD_DEFAULT)]);
}
...
if ((int) $mainDb->query('SELECT COUNT(*) FROM app_groups')->fetchColumn() === 0) {
    ...
    $assign = $mainDb->prepare('INSERT OR IGNORE INTO app_user_groups (user_id, group_id) VALUES (?, ?)');
    $assign->execute([1, $managerId]);
    $assign->execute([2, $staffId]);
}

/** Project user adapted to Prefab Auth and Permissions contracts. */
...
/** Hydrates project users and their project-owned group memberships. */
...
/*
 ...
 |--------------------------------------------------------------------------
 |
 | Permission definitions live in a separate PHP template. To demonstrate the
 | portable JSON alternative, change the filename to permissions.json.
 */
PrefabConfig::set([
    'database' => $mainDb,
    'modules' => [
        ...
        'permissions' => [
            ...
            // Alternative: __DIR__ . '/config/permissions.json',
        ],
    ],
]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'create_group':
            ...
            if ($groupName !== '') {
                $statement = $mainDb->prepare('INSERT OR IGNORE INTO app_groups (name) VALUES (?)');
                $statement->execute([$groupName]);
                $message = 'Group created.';
            }
            break;
        case 'assign_group':
            $statement = $mainDb->prepare('INSERT OR IGNORE INTO app_user_groups (user_id, group_id) VALUES (?, ?)');
            $statement->execute([(int) $_POST['user_id'], (int) $_POST['group_id']]);
            $message = 'User added to group.';
            break;
        case 'remove_group':
            $statement = $mainDb->prepare('DELETE FROM app_user_groups WHERE user_id = ? AND group_id = ?');
            $statement->execute([(int) $_POST['user_id'], (int) $_POST['group_id']]);
        ...
        case 'user_clear':
            ...
            if ($action === 'user_clear') {
                ...
                $permissions->set('user', $userId, $permission, $action === 'user_allow');
            }
        ...
        case 'group_clear':
            ...
            if ($action === 'group_clear') {
                ...
                $permissions->set('group', $groupId, $permission, $action === 'group_allow');
            }
...
$selectedId = (int) ($_GET['user'] ?? $currentUser?->authId() ?? $allUsers[0]->id ?? 1);
...
if ($selectedUser instanceof PermissionSubjectInterface) {
    foreach ($selectedUser->permissionGroupIds() as $groupId) {
        $selectedGroups[] = ['id' => $groupId, 'name' => $groupNames[(int) $groupId] ?? "Group #{$groupId}"];
    }
}
...
function sourceLabel(object $result, array $groupNames): string
{
    ...
    $ids = $result->groups ?: $result->deniedGroups;
...
<nav class="navbar navbar-dark bg-dark"><div class="container"><span class="navbar-brand">Tihloh Prefab — Database Integration Test</span></div></nav>
...
<div class="row g-4">
<div class="col-lg-4">
<div class="card mb-3"><div class="card-header fw-semibold">Authentication</div><div class="card-body">
<?php if ($auth->check()): ?><p>Signed in as <strong><?= e($currentUser?->name ?? $auth->id()) ?></strong></p><form method="post"><button name="action" value="logout" class="btn btn-outline-danger w-100">Sign out</button></form>
...
<div class="col-lg-8">
<div class="card mb-4"><div class="card-header fw-semibold">Users</div><div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>ID</th><th>Name</th><th>Email</th><th></th></tr></thead><tbody><?php foreach ($allUsers as $user): ?><tr><td><?= e($user->id) ?></td><td><?= e($user->name) ?></td><td><?= e($user->email) ?></td><td class="text-end"><a class="btn btn-sm btn-outline-secondary" href="?user=<?= e($user->id) ?>">Manage</a></td></tr><?php endforeach; ?></tbody></table></div></div>
<?php if ($selectedUser): ?><div class="card mb-4"><div class="card-header fw-semibold">Manage <?= e($selectedUser->name) ?></div><div class="card-body">
<form method="post" class="row g-2 mb-4"><input type="hidden" name="user_id" value="<?= e($selectedUser->id) ?>"><div class="col"><input class="form-control" name="name" value="<?= e($selectedUser->name) ?>"></div><div class="col-auto"><button name="action" value="rename" class="btn btn-outline-primary">Rename</button></div></form>
<h6>Groups</h6><div class="d-flex flex-wrap gap-2 mb-3"><?php if (!$selectedGroups): ?><span class="text-muted">No groups</span><?php endif; ?><?php foreach ($selectedGroups as $group): ?><form method="post"><input type="hidden" name="user_id" value="<?= e($selectedUser->id) ?>"><input type="hidden" name="group_id" value="<?= e($group['id']) ?>"><button name="action" value="remove_group" class="btn btn-sm btn-secondary"><?= e($group['name']) ?> ×</button></form><?php endforeach; ?></div>
<form method="post" class="row g-2 mb-4"><input type="hidden" name="user_id" value="<?= e($selectedUser->id) ?>"><div class="col"><select class="form-select" name="group_id"><?php foreach ($groups as $group): ?><option value="<?= e($group['id']) ?>"><?= e($group['name']) ?></option><?php endforeach; ?></select></div><div class="col-auto"><button name="action" value="assign_group" class="btn btn-outline-secondary">Add to group</button></div></form>
<h6>Effective Permissions</h6><div class="table-responsive"><table class="table table-sm align-middle"><thead><tr><th>Permission</th><th>Effective</th><th>Source</th><th>User Override</th></tr></thead><tbody><?php foreach ($permissions->definitions() as $id => $definition): $result = $resolvedPermissions[$id]; ?><tr><td><?= e($definition['name'] ?? $id) ?><br><code><?= e($id) ?></code></td><td><span class="badge text-bg-<?= $result->allowed ? 'success' : 'danger' ?>"><?= $result->allowed ? 'ALLOW' : 'DENY' ?></span></td><td><?= e(sourceLabel($result, $groupNames)) ?></td><td><form method="post" class="btn-group btn-group-sm"><input type="hidden" name="user_id" value="<?= e($selectedUser->id) ?>"><input type="hidden" name="permission" value="<?= e($id) ?>"><button name="action" value="user_allow" class="btn btn-outline-success">Allow</button><button name="action" value="user_deny" class="btn btn-outline-danger">Deny</button><button name="action" value="user_clear" class="btn btn-outline-secondary">Inherit</button></form></td></tr><?php endforeach; ?></tbody></table></div>
</div></div><?php endif; ?>
<div class="card mb-4"><div class="card-header fw-semibold">Group Permissions</div><div class="card-body"><p class="small text-muted">Group values are inherited by members unless a user override exists.</p><?php foreach ($groups as $group): $groupPermissions = $permissions->overridesFor('group', $group['id']); ?><div class="border rounded p-3 mb-3"><h6><?= e($group['name']) ?></h6><div class="table-responsive"><table class="table table-sm mb-0"><tbody><?php foreach ($permissions->definitions() as $id => $definition): ?><tr><td><?= e($definition['name'] ?? $id) ?></td><td><?php if (array_key_exists($id, $groupPermissions)): ?><span class="badge text-bg-<?= $groupPermissions[$id] ? 'success' : 'danger' ?>"><?= $groupPermissions[$id] ? 'ALLOW' : 'DENY' ?></span><?php else: ?><span class="badge text-bg-secondary">INHERIT DEFAULT</span><?php endif; ?></td><td class="text-end"><form method="post" class="btn-group btn-group-sm"><input type="hidden" name="group_id" value="<?= e($group['id']) ?>"><input type="hidden" name="permission" value="<?= e($id) ?>"><button name="action" value="group_allow" class="btn btn-outline-success">Allow</button><button name="action" value="group_deny" class="btn btn-outline-danger">Deny</button><button name="action" value="group_clear" class="btn btn-outline-secondary">Clear</button></form></td></tr><?php endforeach; ?></tbody></table></div></div><?php endforeach; ?></div></div>
<div class="card"><div class="card-header d-flex justify-content-between"><span class="fw-semibold">Activity Logs <span class="badge text-bg-secondary"><?= count($technicalLogs) ?></span></span><span class="small text-muted">separate logs.sqlite</span></div><div class="card-body pb-0"><ul class="nav nav-tabs"><li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#humanLogs">User Friendly</button></li><li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#technicalLogs">Technical / Audit</button></li></ul></div><div class="tab-content"><div class="tab-pane fade show active" id="humanLogs"><div class="list-group list-group-flush"><?php foreach ($humanLogs as $log): ?><div class="list-group-item py-3"><div class="d-flex justify-content-between gap-3"><div><div class="fw-semibold"><?= e($log['summary']) ?></div><?php foreach ($log['details'] as $detail): ?><div class="small text-muted"><?= e($detail['field']) ?>: <?= e($detail['old']) ?> → <strong><?= e($detail['new']) ?></strong></div><?php endforeach; ?></div><small class="text-muted text-nowrap"><?= e($log['when'] ?? '') ?></small></div></div><?php endforeach; ?></div></div><div class="tab-pane fade" id="technicalLogs"><div class="table-responsive"><table class="table table-hover table-sm mb-0"><thead><tr><th>#</th><th>Action</th><th>Actor</th><th>Subject</th><th>Message</th></tr></thead><tbody><?php foreach ($technicalLogs as $log): ?><tr><td><?= e($log['id']) ?></td><td><code><?= e($log['action']) ?></code></td><td><?= e($log['actor_id'] ?? '—') ?></td><td><code><?= e($log['subject_type']) ?> #<?= e($log['subject_id'] ?? '—') ?></code></td><td><?= e($log['message'] ?? '') ?></td></tr><?php endforeach; ?></tbody></table></div></div></div></div>
</div></div>
</main>
